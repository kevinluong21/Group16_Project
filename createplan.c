					 ...
					 Plan *inner_plan)
{
	...
	inner_hash_plan = make_hash(inner_plan); //CSI3130: name modified to represent that it returns inner hash plan node
	outer_hash_plan = make_hash(outer_plan); //CSI3130: added so that it returns outer hash plan node
