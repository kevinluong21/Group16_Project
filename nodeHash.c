...
ExecHash(HashState *node)
{
	...
	for (;;)
	{
		...
		if (TupIsNull(slot))
			break;
		//CSI3130 LINES tagged with NodeErrorHandling: Those lines don't work because hashtable is a NULL pointer
		//CSI3130 When we'll be able to pass a proper node to ExecHash, they will work with no issue
		
		if (hashtable){hashtable->totalTuples += 1;}else{} //CSI3130 NodeErrorHandling
		/* We have to compute the hash value */
		econtext->ecxt_innertuple = slot;
		hashvalue = ExecHashGetHashValue(hashtable, econtext, hashkeys);
		if (hashtable){ExecHashTableInsert(hashtable, ExecFetchSlotTuple(slot), hashvalue);}else{}//CSI3130 NodeErrorHandling
	}
