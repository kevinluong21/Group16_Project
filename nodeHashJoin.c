...
ExecHashJoin(HashJoinState *node)
{
	 ...
	 */
	inner_hashtable = node->hj_InnerHashTable; //CSI3130: access and save the inner hash table
	outer_hashtable = node->hj_OuterHashTable; //CSI3130: added ability to access and save outer hash table as well
    // CSI3530 and CSI3130 ...
	...
	if (inner_hashtable == NULL) //CSI3530 and CSI3130: build a hash table on inner relation
	{
		 ...
		 */
		inner_hashNode->inner_hashtable = inner_hashtable;
	...
	if (outer_hashtable == NULL) //CSI3530 and CSI3130: build a hash table on outer relation
	{
		 ...
		 * execute the Hash node, to build the hash table
		 */
		outer_hashNode->outer_hashtable = outer_hashtable;
